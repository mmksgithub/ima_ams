...
const getAllMembers = async () => {
  try {
    const response = await axios.get(API_URL + 'getAllMembers');
    return response.data;
  } catch (error) {
...
const getAppliedMembers = async () => {
  try {
    const response = await axios.get(API_URL + 'getAppliedMembers');
    return response.data;
  } catch (error) {
    console.log('getAppliedMembers error', error.message);
    throw error;
  }
};
